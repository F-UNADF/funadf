class Api::ProfileController < ApiController
  def show
    phases           = current_user.phases.joins('JOIN structures AS church ON church.id = careers.church_id').select('careers.id, careers.start_at, careers.end_at, careers.church_id, CONCAT(church.name, "(", church.town, ")") AS church_name, church.*, careers.function').order(start_at: :desc)
