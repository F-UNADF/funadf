  ...
  <v-card v-else>
    ...
    <v-card-text>
      <v-tabs color="primary" class="mb-3" align-tabs="center" v-model="tab">
        ...
        <v-tab value="reconnaissances" v-if="!isSelf">Reconnaissances</v-tab>
        <v-tab value="parcours">Parcours</v-tab>
        <v-tab value="responsabilites" v-if="!isSelf">Responsabilités nationales</v-tab>
        <v-tab value="cotisations" v-if="!isSelf">Cotisations</v-tab>
        <v-tab value="roles" v-if="!isSelf">Roles globals</v-tab>
        <v-tab value="security">Securité</v-tab>
        <v-tab value="danger-zone" variant="flat" color="red" prepend-icon="mdi-alert" v-if="!isSelf">
          Danger zone
        </v-tab>
      </v-tabs>
      ...
      <v-window v-model="tab">
        <v-window-item key="infos" value="infos">
          <v-row>
            <v-col cols="12" md="3">
              <div class="logo w-50 mx-auto">
                <v-img v-if="this.editedItem && this.editedItem.user.id !== null" style="border-radius: 100%;"
                  :src="'/avatars/' + this.editedItem.user.id + '.png?cache=' + imgCache"
          ...
          <v-btn color="info" @click="sendInvitation()" class="mr-3"
            v-if="editedItem.user && editedItem.user.invitation_accepted_at === null && !isSelf">
        ...
        <v-window-item key="responsabilites" value="responsabilites">
          <v-row>
            <v-col cols="12" sm="11">
              <v-row class="bg-grey-lighten-4 pa-5 rounded elevation-1 mb-3" v-if="editedItem.responsabilities.length > 0"
                v-for="phase in editedItem.responsabilities" :key="phase.id" justify="center">
                <v-col cols="12" sm="5">
...
export default {
  ...
  computed: {
    ...
    isSelf() {
      return this.item && this.item.user && this.item.user.id === this.user.id;
    },
  },
  methods: {
    close() {
